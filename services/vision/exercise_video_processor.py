...
class VideoProcessorClass(VideoProcessorBase):
    ...
    def set_exercise(self, exercice_type):
        with self._lock:
            self._exercise_type = exercice_type
    ...
    def _draw_processing_error(self, img):
        cv2.putText(
            img,
    ...
    def recv(self, frame):
        flipped = cv2.flip(frame.to_ndarray(format="bgr24"), 1)

        image = np.ascontiguousarray(flipped, dtype=np.uint8)
        try:
            self.frame_count += 1

            if self.frame_count % self.PROCESS_EVERY_N_FRAMES != 0:
                if self._latest_landmarks:
                    self._draw_skeleton(image, self._latest_landmarks)
            ...
            self._latest_landmarks = None
            self._draw_processing_error(image)
